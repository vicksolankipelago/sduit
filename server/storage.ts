...
export class DatabaseStorage implements IStorage {
  ...
  async createJourney(journey: InsertJourney): Promise<Journey> {
    ...
    await db.insert(journeyVersions).values({
      ...
      voice: created.voice,
      voiceEnabled: created.voiceEnabled ?? true,
      ttsProvider: created.ttsProvider || "elevenlabs",
      elevenLabsConfig: created.elevenLabsConfig,
  ...
  async updateJourney(journeyId: string, updates: Partial<InsertJourney>, userId: string, changeNotes?: string): Promise<Journey | undefined> {
    ...
    if (updated) {
      ...
      await db.insert(journeyVersions).values({
        ...
        ttsProvider: updated.ttsProvider || "elevenlabs",
