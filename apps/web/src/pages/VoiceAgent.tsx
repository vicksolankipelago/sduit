...
/**
 * System tool schemas for ElevenLabs tool overrides.
 * These are passed as overrides.agent.prompt.tools when starting an ElevenLabs session
 * so the LLM knows what client tools are available, regardless of dashboard configuration.
 * 
 * Each tool is type: "client" because they execute on the client side (not as webhooks).
 * The handlers for these tools are defined in elevenLabsClientTools below.
 */
const ELEVENLABS_SYSTEM_TOOLS: Array<{
  type: 'client';
  name: string;
  description: string;
  ...
  },
  {
    ...
    description: 'Navigate directly to a specific screen by its ID.',
    parameters: {
      type: 'object',
      properties: {
        screen_id: {
  ...
  },
  {
    type: 'client',
    name: 'switch_agent',
    description: 'Switch the conversation to a different agent.',
    parameters: {
      type: 'object',
      properties: {
        agent_id: {
          type: 'string',
          description: 'The agent ID to switch to',
        },
        agent_name: {
          type: 'string',
          description: 'The agent name to switch to (alternative to agent_id)',
        },
      },
      required: [],
    },
  },
  {
    ...
    description: 'Transfer the conversation to a different agent (alias for switch_agent).',
    parameters: {
      type: 'object',
      properties: {
        agent_id: {
        ...
        agent_name: {
          type: 'string',
          description: 'The agent name to transfer to (alternative to agent_id)',
        },
      },
      required: [],
    },
  ...
  {
    type: 'client',
    name: 'screen_out_participant',
    description: 'Screen out a research participant who does not qualify. Ends the call and redirects to the screen-out URL.',
    parameters: {
      type: 'object',
      properties: {
        reason: {
          type: 'string',
          description: 'Reason for screening out the participant',
        },
      },
      required: [],
    },
...
- Respond promptly when you hear the counsellor speak`,
        ...
        await connect({
          ...
          // Override ElevenLabs agent tools with our system tool schemas
          // This ensures the LLM knows about all client tools regardless of dashboard config
          toolOverrides: ELEVENLABS_SYSTEM_TOOLS,
          // NOTE: clientTools are passed to useElevenLabsSession hook, not connect()
        });
