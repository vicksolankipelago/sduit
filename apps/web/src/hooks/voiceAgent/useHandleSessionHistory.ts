...
export function useHandleSessionHistory() {
  ...
  const { triggerFunctionUI } = useAgentUI();
  ...
  function handleAgentToolStart(details: any, _agent: any, functionCall: any) {
    ...
    // Show function call UI using the new AgentUI system
    if (function_name && function_args) {
      // Try to get agent name from the details context
      const agentName = details?.agent?.name || _agent?.name;
      triggerFunctionUI(function_name, maybeParseJson(function_args), agentName);
    }
  }
