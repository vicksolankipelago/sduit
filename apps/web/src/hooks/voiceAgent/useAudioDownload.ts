...
import { convertWebMBlobToWav } from "../../lib/voiceAgent/audioUtils";
...
function useAudioDownload() {
  ...
  // Track microphone stream so we can release it on stop.
  const micStreamRef = useRef<MediaStream | null>(null);
  // Track the audio context used to merge streams.
  ...
  const startRecording = async (remoteStream: MediaStream) => {
    ...
    const destinationStream = destination.stream;
    ...
    try {
      const mediaRecorder = new MediaRecorder(destinationStream, options);
